...
export default function BulkSMSSend() {
      ...
      complete: (results) => {
        if (results.data && results.data.length > 0) {
          if (results.data.length > maxRows) {
            setError(t('sms.csvRowsExceeded', { rows: maxRows }))
            return
          }
  ...
  return (
    <div className='space-y-8'>
      <Card>
        <CardHeader>
          <CardTitle>{t('sms.bulkTitle')}</CardTitle>
          <CardDescription>
        ...
        <CardContent className='space-y-8'>
          <section>
            <h2 className='text-lg font-semibold mb-2'>{t('sms.uploadCsv')}</h2>
            <p className='text-sm text-gray-500 mb-4'>
              {t('sms.uploadCsvDescription', {
            ...
            {csvData.length > 0 && (
              <p className='mt-2 text-sm text-green-600'>
                {t('sms.csvUploaded', { rows: csvData.length })}
              </p>
            )}
          </section>
          ...
          >
            ...
            <p className='text-sm text-gray-500 mb-4'>
              {t('sms.configureSmsDescription')}
            </p>

            {/* select device to send SMS from   */}
            <div>
              <Label htmlFor='device-select'>{t('sms.selectDevice')}</Label>
              <Select
                onValueChange={(value) => {
            ...
            {availableSims.length > 1 && (
              <div>
                <Label htmlFor='sim-select'>{t('sms.selectSim')}</Label>
                <Select
                  onValueChange={(value) =>
                ...
                >
                  <SelectTrigger id='sim-select'>
                    <SelectValue placeholder={t('sms.selectSim')} />
                  </SelectTrigger>
                  <SelectContent>
                    {availableSims.map((sim) => (
                      ...
                      >
                        {sim.displayName || t('sms.sim')} ({sim.subscriptionId})
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
            ...
            <div className='space-y-4'>
              <div>
                ...
                >
                  <SelectTrigger id='recipient-column'>
                    <SelectValue placeholder={t('sms.selectColumn')} />
                  </SelectTrigger>
                  <SelectContent>
                    {columns.map((column) => (
                      <SelectItem
              ...
              <div>
                ...
                <Textarea
                  id='message-template'
                  placeholder={t('sms.messageTemplatePlaceholder')}
          ...
          >
            <h2 className='text-lg font-semibold mb-2'>
              {t('sms.messagePreview')}
            </h2>
            <p className='text-sm text-gray-500 mb-4'>
              {t('sms.messagePreviewDescription')}
            </p>
            <div className='space-y-4'>
              <div>
                <Label htmlFor='preview-recipient'>
                  {t('sms.selectPreviewRecipient')}
                </Label>
                <Select
                ...
                >
                  <SelectTrigger id='preview-recipient'>
                    <SelectValue placeholder={t('sms.selectRecipient')} />
                  </SelectTrigger>
                  <SelectContent>
                    {csvData
                      .map((row, index) => {
              ...
              <div>
                <Label htmlFor='message-preview'>{t('sms.preview')}</Label>
                <Textarea
                  id='message-preview'
          ...
          {isSendingBulkSMSuccess && (
            <Alert variant='default' className='mt-4'>
              ...
              <AlertDescription>{t('sms.bulkSent')}</AlertDescription>
            </Alert>
          )}
          ...
          >
            ...
            {isSendingBulkSMS ? t('sms.sending') : t('sms.sendBulk')}
          </Button>
        </CardContent>
      </Card>
    </div>
